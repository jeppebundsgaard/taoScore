...
// Platform themes
use dpu\taoScore\model\theme\TaoscoreDefaultTheme;
...
foreach($itemThemes as $themeName => $isDefault){
    ...
    ThemeRegistry::getRegistry()->registerTheme(
        $themeId,
        $themeName,
        implode(DIRECTORY_SEPARATOR, array('taoScore', 'views', 'css', 'themes', 'items', $pathFragment, 'theme.css')), array('items')
    );
}
...
$themeService->setTheme(new TaoscoreDefaultTheme());
